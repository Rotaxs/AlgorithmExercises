...
void solve() {
    ...
    auto findKthEmpty = [&](int a, int k) {
        int l = a - 1, r = n;
        while ((l + 1) != r) {
            int mid = l + ((r - l) >> 1);
            int zeros = mid - a + 1 - seg.query(a, mid);
            if (zeros >= k)
                r = mid;
            else
                l = mid;
    ...
    while (m--) {
        ...
        if (k == 1) {
            int a, f;
            cin >> a >> f;
            int rem = (n - a) - seg.query(a, n - 1);
            if (rem == 0) {
                cout << "Can not put any one." << endl;
                continue;
            }
            int start = findKthEmpty(a, 1);
            int end = findKthEmpty(a, min(f, rem));
            seg.rangeSet(start, end, 1);
            cout << start << ' ' << end << endl;
        } else if (k == 2) {
            int a, b;
            cin >> a >> b;
