...
`set` 和 `multiset` 底层均基于红黑树实现，区别在于 `set` 不允许集合中有相同的元素，而 `multiset` 可以

- `insert()` 插入指定元素 $O(\log n)$
- `erase()` 删除元素 $O(\log n)$
  - 如果传入的是一个值，则删除这个值 $O(\log n + num)$
  - 如果传入的是一个迭代器，则是删除这个位置上的值
- `upper_bound()` 和 `lower_bound`： $O(\log n)$
  -  `upper_bound()` 返回大于某个数的第一个值的迭代器
  -  `lower_bound()` 返回大于等于某个数的第一个值的迭代器
- `count()` 返回某个值在集合中出现的次数 $O(\log n + num)$
- `size()` 返回集合中元素的个数 $O(1)$
- `empty()` 判断集合是否为空 $O(1)$
- `begin()` 和 `end()` $O(1)$
  - `begin()` 获取最小值的迭代器
...
- `count()` 返回某个 key 出现的次数
- `lower_bound` 和 `upper_bound`：二分查找 key
- `erase()` 按照 key/迭代器删除元素
- `begin()` 和 `end()`
- `clear()`
- `size()` 和 `empty()`
...
给定一个数组/字符串，要求无重复元素的字数粗/子串的某个属性（比如这里求最大长度）

核心是用一个 `lastSeen` 记录某个元素上一次出现的位置

然后用滑动窗口的思路，当右边界出现过时，就更新左边界，以保证 `[l, r]` 内始终没有重复元素
...
这里利用 `unordered_map` 可以处理数据较大的情况，对于较小的数据或一般字符，可以直接使用数组
...
- 时间复杂度：合并和查询均为 $O(\alpha(n))$，其中 $\alpha(n)$ 是反阿克曼函数，其在 $n$ 极大的时候，函数值也是一个非常小的常数
- 空间复杂度：$O(n)$
...
用 `hasSet` 表示是否有 `set` 标记

需要注意如果某个节点若同时存在 `set` 和 `add` 标记，需要先 `set` 再 `add`
...
一个数 $m$ 的格雷码 $g_m = m \oplus \lfloor \frac{m}{2} \rfloor$

而格雷码转为原二进制码可以用如下的计算方法

$$
\begin{array}{rcccc}
\text{Bit:} & \text{Bit3} & \text{Bit2} & \text{Bit1} & \text{Bit0} \\
\text{格雷码:} & \mathbf{1} & \mathbf{1} & \mathbf{0} & \mathbf{1} \\[2pt]
& \downarrow & \nearrow \downarrow \oplus & \nearrow \downarrow \oplus & \nearrow \downarrow \oplus \\[2pt]
\text{二进制:} & \mathbf{1} & \mathbf{0} & \mathbf{0} & \mathbf{1}
\end{array}
$$
...
一维 $k$ 阶的递推关系，如一维二阶递推关系 $F_n = F_{n - 1} + 2F_{n - 2}$，存在通项关系 $\begin{bmatrix}F_n & F_{n - 1}\end{bmatrix} = \begin{bmatrix}F_1 & F_0\end{bmatrix} \times A^{n - 1}$

最容易理解的是等比数列 $f_n = af_{n - 1}$，那么 $f_n = f_{1}\times a^{n - 1}$

>  [!important]
> 一般来说，一维 $k$ 阶的矩阵的初始矩阵是一个 $1 \times k$ 的矩阵，常数矩阵是 $k \times k$ 的矩阵
> 例如一维 $k$ 阶的矩阵的初始矩阵为 $\begin{bmatrix}F_{k - 1} & F_{k - 2} & \cdots & F_1 & F_0\end{bmatrix}$，常数矩阵待定系数列方程即可

为了保证矩阵最小、跑得最快，一般遵循这个维度计算公式：最小总维度 = (状态项的最大跨度之和) + (附加项的维度)

具体拆解如下

- 第一部分：状态项的最大跨度看等式右边，每个变量最多往回倒推了几项
  - $f_n = f_{n-1}$： 只需要上一项，贡献 1 维（存 $f_n$）
  - $f_n = f_{n-1} + _{n-2}$：倒推了 2 项，贡献 2 维（存 $f_n, f_{n-1}$）
  - $A_n = A_{n-1} + B_{n-3}$：$A$ 倒推了 1 项，$B$ 倒推了 3 项。总共贡献 $1 + 3 =$ 4 维
- 第二部分：附加项只要等式右边出现了非状态变量，就得给它分配空间
  - 常数 $C$：贡献 1 维（永远放个 $1$）
  - 一次项 $n$：贡献 2 维（放 $n$ 和 $1$）
  - 平方项 $n^2$：贡献 3 维（放 $n^2, n, 1$）
  - 指数项 $C^n$：贡献 1 维（放 $C^n$）
...
> 注：上面例题的推导都是基于列向量，在写代码时一般使用行向量，因此要将常数矩阵的行列互换

##### 矩阵快速幂加速 k 维一阶递推

比如在二维递推中，某一维的状态数较少，可以把这一维所有的状态全部列成一个向量。然后这个新的向量就可以按照一维 k 阶的方式进行状态转移

还有一些是类似下面的情况

【例1】有一个 $1 \times N$ 的长条土地，在这 $N$ 个格子中种树或留空，要求不能有任意两棵树相邻，要求总方案数

首先考虑这样一件事情：对于每个格子来说，有两种状态，种树（记为 1），留空（记为 0），如果在下面的表格中以列表示第 `i`  列的格子的状态，行表示第 `i + 1` 列格子的状态，显然得到下面的能否“实现”的真值表
...
我们把这个真值表抽取成一个矩阵 $\begin{bmatrix} 1 & 1 \\ 1 & 0 \end{bmatrix}$，此时如果有一个初始状态比如 $\begin{bmatrix} 1 & 1 \end{bmatrix}$（分别表示当前格子种树和不种树都有一种方案），那么

$$
\begin{bmatrix} 1 & 1 \end{bmatrix} \times \begin{bmatrix} 1 & 1 \\ 1 & 0 \end{bmatrix} = \begin{bmatrix} 2 & 1 \end{bmatrix}
$$

我们可以根据矩阵乘法的定义看看这个矩阵 $\begin{bmatrix} 2 & 1 \end{bmatrix}$ 是什么意思

首先是 $2$，它等于 $1 \times 1 + 1 \times 1$，第一个 $1 \times 1$，表示第一格不种树的情况下，第二格也不种树；第二个 $1 \times 1$ 表示第一格种树的情况下，第二格不种树，这就表示第二格不种树的方案总数

同理，这个 $1$ 可以写成 $1 \times 1 + 1 \times 0$，表示第一格不种树的情况下，第二格可以种树；和第一格种树的情况下，第二格不能种树

因此，只要不断乘这个转移矩阵，就能得到任意到达任意一格的种树方案数。用矩阵快速幂即可很快解决
...
在**树**的 dfs/bfs 遍历中，我们可以不使用 `vis` 数组，而引入新的 `fa` 变量表示父节点来避免死循环
